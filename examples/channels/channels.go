...
func doit() {
	...
	responseChannel := make(chan int) // construct blocking channel
	defer close(responseChannel)      // knows when all writers are done, so can close
	...
	x, y := <-responseChannel, <-responseChannel // receive twice from channels
